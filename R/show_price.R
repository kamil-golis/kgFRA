#' A function that plots quotations for a given stock.
#'
#' @author Kamil Golis
...
#' @return A plot depicting price levels of the stock is returned.
#' @examples
#' show_price('AAPL')
#' show_price('MSFT', '2020-01-01', '2022-03-31', 'open')
#' @export

show_price <- function(symbol, start_date=Sys.Date()-365, end_date=Sys.Date(), price_type = 'close') {
  ...
  if (is.null(stock)) {
    ...
    plot(prices,
         main = paste0("Quotations of ", symbol, ' ($)'))
  }
}
